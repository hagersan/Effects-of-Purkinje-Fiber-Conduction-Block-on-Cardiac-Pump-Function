...
class Constant_definitions(object):
    ...
    def default_parameters(self):
        ...
        return {
            "alpha": Constant(0.01),  # 0.1 in 'a' in NashPanfilov, 0.01 in Gok_Kuhl
...
class biventricle_mesh(object):
    ...
    input: mesh, facet, edge, matid, fibre file, baseid
    output: mesh
    """
    ...
    def __init__(self, params, SimDet):
        ...
        casename_marked = self.parameters["casename_marked"],
        ...
        if "fiber_fspace" in list(SimDet.keys()) and "fiber_fspace_deg" in list(SimDet.keys()):
            ...
            VQuadelem = VectorElement(SimDet["fiber_fspace"], self.mesh.ufl_cell(), degree=SimDet["fiber_fspace_deg"],)
        else:
        ...
        self.matid = MeshFunction("size_t", self.mesh, self.mesh.topology().dim(), self.mesh.domains())
        if f.has_dataset(casename + "/" + "matid"):
            f.read(self.matid, casename + "/" + "matid")
        else:
            self.matid.set_all(0)

        self.baseid = MeshFunction("size_t", self.mesh, self.mesh.topology().dim(), self.mesh.domains())
        if f.has_dataset(casename + "/" + "base_id"):
            f.read(self.baseid, casename + "/" + "base_id")
        else:
            self.baseid.set_all(0)              

        self.AHAid = MeshFunction("size_t", self.mesh, self.mesh.topology().dim(), self.mesh.domains())
        if f.has_dataset(casename + "/" + "AHAid"):
            f.read(self.AHAid, casename + "/" + "AHAid")
        else:
            self.AHAid.set_all(0)

        self.ischemia_mask = load_ischemia_mask(self.mesh, self.parameters["directory"], self.parameters.get("casename_marked", None))    
        # vals = [int(ischemia_mask[cell.index()]) for cell in cells(mesh)]
        # if mesh.mpi_comm().rank == 0:
        #     print("[EP mask sanity]")
        #     print(" unique:", set(vals))
        #     print(" ischemic:", sum(v == 1 for v in vals))
        #     print(" total:", len(vals))
        # pdb.set_trace()

        EpiBCid = MeshFunction("size_t", self.mesh, self.mesh.topology().dim() - 1, self.mesh.domains())
        ...
        dx = dolfin.dx(self.mesh, subdomain_data=self.matid, metadata={"quadrature_degree": deg})
        # dx = dolfin.dx(self.mesh, subdomain_data=self.AHAid)
        ds = dolfin.ds(self.mesh, subdomain_data=self.facetboundaries, metadata={"quadrature_degree": deg},)
        self.dx = dx
        self.ds = ds
    ...
    def __init__(self, biVMesh, params, SimDet):
        ...
        if isincomp:
            if self.isLV:
                ...
                W = FunctionSpace(
                    mesh, MixedElement([Velem, Qelem, Relem, Relem, VRelem])
                )
...
class FHN(object):
    ...
    def __init__(self, biVMesh, params):
        ...
        vtkfile_Ta_fhn = File(
            outputfolder + folderName + caseID + "solution_Ta_fhn.pvd"
        )

        vtkfile_q = File(outputfolder + folderName + caseID + "solution_q.pvd")
        fdataECG = open(
            outputfolder + folderName + caseID + "IntegralCharge_.txt", "w", 0
        )
    ...
    def FHN_F_J(self, J_fhn=None, C_fhn=None):
        ...
        F_FHN = (
            ...
            + 1 / J_n * dot(J_n * inv(C_n) * Dmat * grad(phi), grad(phi_test)) * dx
            - f_phi * phi_test * dx
            + ((r - r_n) / k) * r_test * dx
            - f_r * r_test * dx
...
class State_Variables(object):
    ...
    def update_state(self, pv_o, circuit_o, t_a):
        ...
        if (
            pv_o.LVP_cav * 0.0075 < circuit_o.Pmv
            and self.isLVrelaxing
            and pv_o.LVPfilling_step == 0
        ):
            self.isLVfill = 1
            ...
            pv_o.LVPfilling_step = (
                (pv_o.LVEDP - pv_o.LVP_cav) / (self.BCL - t_a.t_a) * self.dt.dt
            )
            pv_o.LVVfilling_step = (
                (pv_o.LVEDV - pv_o.LVESV) / (self.BCL - t_a.t_a) * self.dt.dt
            )

        if not self.isLV:
            if (
                pv_o.RVP_cav * 0.0075 < circuit_o.Ptr
                ...
                pv_o.RVPfilling_step = (
                    (pv_o.RVEDP - pv_o.RVP_cav) / (self.BCL - t_a.t_a) * self.dt.dt
                )
                pv_o.RVVfilling_step = (
                    (pv_o.RVEDV - pv_o.RVESV) / (self.BCL - t_a.t_a) * self.dt.dt
                )
...
class Windkessel(object):
    ...
    def calculate_F_Q(self, state_o, pv_o):
        ...
        if self.isLV:
            string_to_print = (
                "Fdiff = " + str(self.normFdiff) + " F[0] = " + str(self.F)
            )
        else:
            string_to_print = (
                "Fdiff = "
...
class exportfiles(object):
    ...
    def removeAllfiles(self):
        ...
        os.system("rm " + outputfolder + folderName + "*.xdmf")
        os.system("rm " + outputfolder + folderName + "*.h5")
    ...
    def opendatafilestreams(self):
        ...
        if MPI.rank(comm) == 0:
            self.fdatalog = open(outputfolder + folderName + "BiV_log.txt", "w", -1)
            ...
            self.fdataIMP2 = open(outputfolder + folderName + "BiV_IMP2.txt", "w", -1)
            self.fdataIMP3 = open(
                outputfolder + folderName + "BiV_IMP_InC.txt", "w", -1
            )

            self.fdataStress = open(
                outputfolder + folderName + "BiV_fiberStress.txt", "w", -1
            )
            self.fdataStrain = open(
                outputfolder + folderName + "BiV_fiberStrain.txt", "w", -1
            )
            self.fdataWork = open(
                outputfolder + folderName + "BiV_fiberWork.txt", "w", -1
            )

            self.fdata_C_Strain = open(
                outputfolder + folderName + "BiV_CStrain.txt", "w", -1
            )
            self.fdata_L_Strain = open(
                outputfolder + folderName + "BiV_LStrain.txt", "w", -1
            )
            self.fdata_R_Strain = open(
                outputfolder + folderName + "BiV_RStrain.txt", "w", -1
            )
    ...
    def openVTKfilestreams(self):
        ...
        self.vtkfile_phi_ref = File(
            outputfolder + folderName + "FHN_coarseRef/phi_ref.pvd"
        )
        ...
        self.vtkfile_IMP_Constraint = File(
            outputfolder + folderName + "IMP_Constraint/IMP_Constraint.pvd"
        )

        self.displacementfile = File(
            outputfolder + folderName + "/deformation/u_disp.pvd"
        )
        self.displacementED_file = File(
            outputfolder + folderName + "/deformation_loadED/u_disp.pvd"
        )

        self.activationFile = File(
            outputfolder + folderName + "/active/activationTime.pvd"
        )
...
def load_ischemia_mask(mesh, directory, casename_marked):
    """
    Load ischemia_mask from marked HDF5 file into a MeshFunction.
    If missing or invalid, returns a zero-filled MeshFunction.
    """

    ischemia = MeshFunction("size_t", mesh, mesh.topology().dim(), 0)

    if casename_marked is None:
        print("⚠️  No casename_marked provided → ischemia disabled.")
        return ischemia

    marked_file = os.path.join(directory, casename_marked + ".hdf5")

    if not os.path.exists(marked_file):
        print(f"⚠️  Marked file not found: {marked_file} → ischemia disabled.")
        return ischemia

    try:
        fm = HDF5File(mesh.mpi_comm(), marked_file, "r")
        fm.read(ischemia, casename_marked + "/ischemia_mask")
        fm.close()
        print(f"Loaded ischemia_mask from {marked_file}")
    except Exception as e:
        print(f"⚠️ Failed loading ischemia_mask → using zeros. Reason: {e}")

    return ischemia



def build_active_indicator(mesh, ischemia_mask, enable_ischemia, label):
    """
    Build DG0 active indicator:
      1.0 = healthy
      0.0 = ischemic
    """
    ...
    if enable_ischemia and ischemia_mask is not None:
        mask_array = np.array(ischemia_mask.array(), dtype=float)
        active.vector()[:] = 1.0 - mask_array

        if mesh.mpi_comm().rank == 0:
            print(
                f"[{label}] Ischemia: {int(mask_array.sum())} / "
                f"{mask_array.size} cells ischemic"
            )
    else:
        active.vector()[:] = 1.0
        if mesh.mpi_comm().rank == 0:
            print(f"[{label}] Ischemia disabled → all cells healthy")
...
# def load_ischemia_mask(mesh, directory, casename_marked):
#     """
#     Load ischemia_mask from marked HDF5 file into a cell-wise MeshFunction.
#     Ischemic = 1, Healthy = 0.
#     Works WITHOUT h5py.
#     """

#     dim = mesh.topology().dim()
#     ischemia = MeshFunction("size_t", mesh, dim, 0)
...
#         h5 = HDF5File(mesh.mpi_comm(), marked_file, "r")
#         h5.read(mvc, casename_marked + "/ischemia_mask")
#         h5.close()

#         ischemia = MeshFunction("size_t", mesh, mvc)
