...
def postprocessdata(IODet, SimDet, cycle=None):
    ...
    for ncycle in range(cycle - 1, cycle):
        ...
        if max(homo_LVP) < 25:
            ...
            EF = (max(homo_LVV) - min(homo_LVV)) / max(homo_LVV) * 100
...
def plothemodynamics_comparison_ind(IODet, SimDet, cycle=None):
    ...
    plt.plot(homo_LVV2, homo_LVP2, linestyle="dashed", label=f"{crt}")

    filename3 = os.path.join(IODet["outputfolder"]+IODet["block"]+"/BiV_PV.txt")
    homo_tptt3, homo_LVP3, homo_LVV3, homo_RVP3, homo_RVV3, homo_Qmv3 = extract_PV(filename3, BCL, ncycle, SimDet)
    plt.plot(homo_LVV3, homo_LVP3, linestyle="dashed",label=f"{block}")
...
def plot_strain_comparison(IODet, SimDet, cycle=None):
    """Compare strain-time curves from two different Ecc.nz, Ell.nz and Err.nz files."""
    BCL = SimDet["HeartBeatLength"]
    block = IODet["block"]   
    crt = IODet["caseID"]
    ...
    Ecc_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"/Ecc/Ecc.npz")
    Ecc_baseline_load = np.load(Ecc_baseline_filename)
    Ecc_baseline = Ecc_baseline_load[Ecc_baseline_load.files[0]]

    Ecc_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"/Ecc/Ecc.npz")
    Ecc_block_load = np.load(Ecc_block_filename)
    Ecc_block = Ecc_block_load[Ecc_block_load.files[0]]

    Ecc_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"/Ecc/Ecc.npz")
    Ecc_case_load = np.load(Ecc_case_filename)
    Ecc_case = Ecc_case_load[Ecc_case_load.files[0]]    

    x = np.arange(len(Ecc_block))    

    plt.plot(x, Ecc_baseline, label=f"Baseline")
    plt.plot(x, Ecc_case, linestyle="dashed", label=f"{crt}")   
    plt.plot(x, Ecc_block, linestyle="dashed", label=f"{block}")    

    hemodynamics_outdirectory = os.path.join(IODet["outputfolder"])
    if not os.path.exists(hemodynamics_outdirectory):
        os.mkdir(hemodynamics_outdirectory)
    ...
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.savefig(os.path.join("./Ecc_comparison.png"))
    plt.clf()    
    ...
    RV_Ecc_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"/Ecc/Ecc_RV.npz")
    RV_Ecc_baseline_load = np.load(RV_Ecc_baseline_filename)
    RV_Ecc_baseline= Ecc_baseline_load[RV_Ecc_baseline_load.files[0]]

    RV_Ecc_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"/Ecc/Ecc_RV.npz")
    RV_Ecc_block_load = np.load(RV_Ecc_block_filename)
    RV_Ecc_block = Ecc_block_load[RV_Ecc_block_load.files[0]]    

    RV_Ecc_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"/Ecc/Ecc_RV.npz")
    RV_Ecc_case_load = np.load(RV_Ecc_case_filename)
    RV_Ecc_case = Ecc_case_load[RV_Ecc_case_load.files[0]]  

    plt.plot(x, RV_Ecc_baseline, label=f"Baseline")
    plt.plot(x, RV_Ecc_case, linestyle="dashed", label=f"{crt}")   
    plt.plot(x, RV_Ecc_block, linestyle="dashed", label=f"{block}")    
    ...
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.savefig(os.path.join("./RV_Ecc_comparison.png"))
    plt.clf()      

    #### Ell LV profile
    plt.figure()

    Ell_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"/Ell/Ell.npz")
    Ell_baseline_load = np.load(Ell_baseline_filename)
    Ell_baseline = Ell_baseline_load[Ell_baseline_load.files[0]]

    Ell_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"/Ell/Ell.npz")
    Ell_block_load = np.load(Ell_block_filename)
    Ell_block = Ell_block_load[Ell_block_load.files[0]]    

    Ell_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"/Ell/Ell.npz")
    Ell_case_load = np.load(Ell_case_filename)
    Ell_case = Ell_case_load[Ell_case_load.files[0]]   


    plt.plot(x, Ell_baseline, label=f"Baseline")
    plt.plot(x, Ell_case, linestyle="dashed", label=f"{crt}")   
    plt.plot(x, Ell_block, linestyle="dashed", label=f"{block}")    
    ...
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.savefig(os.path.join("./Ell_comparison.png"))
    plt.clf()           

    #### Ell RV profile
    plt.figure()

    RV_Ell_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"/Ell/Ell_RV.npz")
    RV_Ell_baseline_load = np.load(RV_Ell_baseline_filename)
    RV_Ell_baseline = Ell_baseline_load[RV_Ell_baseline_load.files[0]]   

    RV_Ell_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"/Ell/Ell_RV.npz")
    RV_Ell_block_load = np.load(RV_Ell_block_filename)
    RV_Ell_block = Ell_block_load[RV_Ell_block_load.files[0]]       

    RV_Ell_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"/Ell/Ell_RV.npz")
    RV_Ell_case_load = np.load(RV_Ell_case_filename)
    RV_Ell_case = Ell_case_load[RV_Ell_case_load.files[0]]     


    plt.plot(x, RV_Ell_baseline, label=f"Baseline")
    plt.plot(x, RV_Ell_case, linestyle="dashed", label=f"{crt}")   
    plt.plot(x, RV_Ell_block, linestyle="dashed", label=f"{block}")    
    ...
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.savefig(os.path.join("./RV_Ell_comparison.png"))
    plt.clf()       

    # #### Err LV profile
    # plt.figure()

    # Err_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"Err/Err.npz")
    # Err_baseline_load = np.load(Err_baseline_filename)
    # Err_baseline = Err_baseline_load[Err_baseline_load.files[0]]

    # Err_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"Err/Err.npz")
    # Err_block_load = np.load(Err_block_filename)
    # Err_block = Err_block_load[Err_block_load.files[0]]    

    # Err_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"Err/Err.npz")
    # Err_case_load = np.load(Err_case_filename)
    # Err_case = Err_case_load[Err_case_load.files[0]]  

    # x = np.arange(len(Err_baseline))    

    # plt.plot(x, Err_baseline, label=f"Baseline")
    # plt.plot(x, Err_case, linestyle="dashed", label=f"{crt}")   
    # plt.plot(x, Err_block, linestyle="dashed", label=f"{block}")    
    ...
    # plt.xlabel("Time point", fontsize=14)
    # plt.ylabel("Strain", fontsize=14)
    # plt.savefig(os.path.join("./Err_comparison.png"))
    # plt.clf()          

    # #### Err RV profile
    # plt.figure()

    # RV_Err_baseline_filename = os.path.join(IODet["outputfolder"]+IODet["directory_baseline"]+"Err/Err_RV.npz")
    # RV_Err_baseline_load = np.load(RV_Err_baseline_filename)
    # RV_Err_baseline = Err_baseline_load[RV_Err_baseline_load.files[0]]   

    # RV_Err_block_filename = os.path.join(IODet["outputfolder"]+IODet["block"]+"Err/Err_RV.npz")
    # RV_Err_block_load = np.load(RV_Err_block_filename)
    # RV_Err_block = Err_block_load[RV_Err_block_load.files[0]]       

    # RV_Err_case_filename = os.path.join(IODet["outputfolder"]+IODet["caseID"]+"Err/Err_RV.npz")
    # RV_Err_case_load = np.load(RV_Err_case_filename)
    # RV_Err_case = Err_case_load[RV_Err_case_load.files[0]]    

    # x = np.arange(len(RV_Err_baseline))    

    # plt.plot(x, RV_Err_baseline, label=f"Baseline")
    # plt.plot(x, RV_Err_case, linestyle="dashed", label=f"{crt}")   
    # plt.plot(x, RV_Err_block, linestyle="dashed", label=f"{block}")    
    ...
    # plt.xlabel("Time point", fontsize=14)
    # plt.ylabel("Strain", fontsize=14)
    # plt.savefig(os.path.join("./RV_Err_comparison.png"))
    # plt.clf()   




def plot_strain_comparison_ind(IODet, SimDet, cycle=None):
    """Compare strain-time curves from baseline, block, and case for Ecc and Ell."""
    ...
    Ecc_baseline = load_npz(os.path.join(IODet["outputfolder"], IODet["directory_baseline"], "Ecc/Ecc.npz"))
    Ecc_block    = load_npz(os.path.join(IODet["outputfolder"], IODet["block"],              "Ecc/Ecc.npz"))
    Ecc_case     = load_npz(os.path.join(IODet["outputfolder"], IODet["caseID"],            "Ecc/Ecc.npz"))

    x = np.arange(start, end)

    plt.plot(x, Ecc_baseline[start:end], label="Baseline")
    plt.plot(x, Ecc_case[start:end], linestyle="dashed", label=f"{crt}")
    plt.plot(x, Ecc_block[start:end], linestyle="dashed", label=f"{block}")

    plt.xticks([320, 360, 400, 440, 480, 520, 560, 600, 640])
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.legend()
    plt.savefig("./Ecc_comparisonind.png")
    plt.clf()
    ...
    RV_Ecc_baseline = load_npz(os.path.join(IODet["outputfolder"], IODet["directory_baseline"], "Ecc/Ecc_RV.npz"))
    RV_Ecc_block    = load_npz(os.path.join(IODet["outputfolder"], IODet["block"],              "Ecc/Ecc_RV.npz"))
    RV_Ecc_case     = load_npz(os.path.join(IODet["outputfolder"], IODet["caseID"],            "Ecc/Ecc_RV.npz"))

    plt.plot(x, RV_Ecc_baseline[start:end], label="Baseline")
    plt.plot(x, RV_Ecc_case[start:end], linestyle="dashed", label=f"{crt}")
    plt.plot(x, RV_Ecc_block[start:end], linestyle="dashed", label=f"{block}")

    plt.xticks([320, 360, 400, 440, 480, 520, 560, 600, 640])
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.legend()
    plt.savefig("./RV_Ecc_comparisonind.png")
    plt.clf()
    ...
    Ell_baseline = load_npz(os.path.join(IODet["outputfolder"], IODet["directory_baseline"], "Ell/Ell.npz"))
    Ell_block    = load_npz(os.path.join(IODet["outputfolder"], IODet["block"],              "Ell/Ell.npz"))
    Ell_case     = load_npz(os.path.join(IODet["outputfolder"], IODet["caseID"],            "Ell/Ell.npz"))

    plt.plot(x, Ell_baseline[start:end], label="Baseline")
    plt.plot(x, Ell_case[start:end], linestyle="dashed", label=f"{crt}")
    plt.plot(x, Ell_block[start:end], linestyle="dashed", label=f"{block}")

    plt.xticks([320, 360, 400, 440, 480, 520, 560, 600, 640])
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.legend()
    plt.savefig("./Ell_comparisonind.png")
    plt.clf()
    ...
    RV_Ell_baseline = load_npz(os.path.join(IODet["outputfolder"], IODet["directory_baseline"], "Ell/Ell_RV.npz"))
    RV_Ell_block    = load_npz(os.path.join(IODet["outputfolder"], IODet["block"],              "Ell/Ell_RV.npz"))
    RV_Ell_case     = load_npz(os.path.join(IODet["outputfolder"], IODet["caseID"],            "Ell/Ell_RV.npz"))

    plt.plot(x, RV_Ell_baseline[start:end], label="Baseline")
    plt.plot(x, RV_Ell_case[start:end], linestyle="dashed", label=f"{crt}")
    plt.plot(x, RV_Ell_block[start:end], linestyle="dashed", label=f"{block}")

    plt.xticks([320, 360, 400, 440, 480, 520, 560, 600, 640])
    plt.xlabel("Time point", fontsize=14)
    plt.ylabel("Strain", fontsize=14)
    plt.legend()
    plt.savefig("./RV_Ell_comparisonind.png")
    plt.clf()
