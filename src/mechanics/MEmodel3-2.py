    ...
    def Problem(self):
        ...
        if not self.ispctrl:
            ...
            if self.isLV or self.iswaorta:
                ...
                Fp = derivative(Fp_lv_me, w_me, wtest_me) + derivative(Fp_rv_me, w_me, wtest_me)
                if self.SimDet.get("fch_fe"):
                    Fp_la_me = uflforms.LAcavitypres()
                    Fp_ra_me = uflforms.RAcavitypres()
                    Fp += derivative(Fp_la_me, w_me, wtest_me) + derivative(Fp_ra_me, w_me, wtest_me)
        ...
        if "springbc" in list(self.SimDet.keys()) and self.SimDet["springbc"]:
            ...
            if self.iswaorta:
                ...
                F3_epi = inner(outer(n_me, n_me) * (epiid_Kadj_coeff * k_spring[0] * u_me + c_damping[0] * (u_me - u_me_n)),
                    v_me,) * (ds_me(epiid) + ds_me(apxid)) + inner((Identity(u_me.ufl_shape[0]) - outer(n_me, n_me))
                    * (epiid_Kadj_coeff * k_spring[1] * u_me + c_damping[1] * (u_me - u_me_n)),
                    v_me,) * (ds_me(epiid) + ds_me(apxid))

                F3 = F3_epi

                if self.SimDet.get("mv_aorta"):

                    F3_aorta_ring = inner(outer(n_me, n_me) * (epiid_Kadj_coeff * k_spring[0] * u_me + c_damping[0] * (u_me - u_me_n)),
                        v_me,) * ds_me(aorta_ring) + inner((Identity(u_me.ufl_shape[0]) - outer(n_me, n_me)) * (
                            epiid_Kadj_coeff * k_spring[1] * u_me + c_damping[1] * (u_me - u_me_n)),
                        v_me,) * (ds_me(aorta_ring))
                ...
                F3_epi = inner(outer(N_me, N_me)* (k_spring[0] * epiid_Kadj_coeff[0] * self.Mesh.poissonF * (u_me - u_me_ED) 
                        + c_damping[0] * (u_me - u_me_n)),v_me,) * (ds_me(epiid)) + inner((Identity(u_me.ufl_shape[0]) - outer(N_me, N_me))
                        * (k_spring[1] * epiid_Kadj_coeff[1] * (u_me - u_me_ED)
                        + c_damping[1] * (u_me - u_me_n)),v_me,) * (ds_me(epiid))
